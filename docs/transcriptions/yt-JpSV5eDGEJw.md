...
> yet what about via multiplayer vehicle desync yes same thing i've said before with, there might be a general networking and performance improvement, in general from the engine update but there are still a lot of optimization plans, so it's not like it's it's there are plans for it, it's always like this always time is against us you know what i mean like we only have so many hours per day to work on stuff and there's so many things that we want to work on so but we we hear you we know it's an issue
