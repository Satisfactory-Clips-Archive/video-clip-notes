---
title: "June 29th, 2021 Livestream Q&A: Please add Achievements & badges on Steam"
date: "2021-06-29"
layout: transcript
topics:
    - "features/buildings/space-elevator"
    - "features/possible-features/third-party-service-integration/achievements"
---
# [June 29th, 2021 Livestream](../2021-06-29.md)
## Q&A: Please add Achievements & badges on Steam
https://www.youtube.com/watch?v=vrBRxuX6YyA
This question was possibly duplicated with a more recent answer: [February 8th, 2022 Livestream Q&A: Are there Achievements planned for Satisfactory?](./yt-i-9UAyVFe6s.md) [https://www.youtube.com/watch?v=i-9UAyVFe6s](https://www.youtube.com/watch?v=i-9UAyVFe6s)


### Topics
* [Features > Buildings > Space Elevator](../topics/features/buildings/space-elevator.md)
* [Features > Possible Features > Third-Party Service Integration > Achievements](../topics/features/possible-features/third-party-service-integration/achievements.md)

### Transcript

> please add achievements and badges on steam we'll add, achievements one day not at the moment though probably for at one point when we know how the game is like if that it's balanced and basically finished, then we'll add achievements then yeah it kind of sucks if we'd add achievements before we knew like for instance if we had achievements for the face for the elevator which used to be like you know send off 500 iron plates which is like nothing now that achievement would mean nothing essentially at this point exactly
