---
title: "May 17th, 2022 Livestream Q&A: What about the Uranium fuel alt?"
date: "2022-05-17"
layout: transcript
topics:
    - "features/equipment/beacon"
    - "features/power-management/nuclear-energy"
    - "features/power-management/nuclear-energy/nuclear-refinement"
    - "satisfactory-updates/released/satisfactory-update-6"
---
# [May 17th, 2022 Livestream](../2022-05-17.md)
## Q&A: What about the Uranium fuel alt?
https://www.youtube.com/watch?v=4j-WwaYx9sQ
This question was possibly replaced with a more recent answer: [May 20th, 2022 Video Beacons](./yt-QYMHs1aTyOs,60.733333333333334,193.31965.md) [https://youtube.com/embed/QYMHs1aTyOs?autoplay=1&start=60&end=194](https://youtube.com/embed/QYMHs1aTyOs?autoplay=1&start=60&end=194)


### Topics
* [Features > Equipment > Beacon](../topics/features/equipment/beacon.md)
* [Features > Power Management > Nuclear Energy](../topics/features/power-management/nuclear-energy.md)
* [Features > Power Management > Nuclear Energy > Nuclear Refinement](../topics/features/power-management/nuclear-energy/nuclear-refinement.md)
* [Satisfactory Updates > Released > Satisfactory Update 6](../topics/satisfactory-updates/released/satisfactory-update-6.md)

### Transcript

> what about the uranium fuel alt so yeah if I didn't say this before but every recipe that involves beacons are probably going to get changed, to not have beacons essentially, I've seen a lot of people speculate that like oh maybe they'll add something new no right now we don't have any plans to anything new to replace beacons with we're probably just going to rebalance how those recipes are I don't remember how many recipes we have that utilize beacons but it's like radar tower it's that alt recipe it's like explorer for the most part like just removing the beacon in most of these recipes we feel is enough I think but I can't I can't speak for certain because mark just started looking into this, so like it's it's actually too early for me to even speculate about that stuff, because I don't know yet
