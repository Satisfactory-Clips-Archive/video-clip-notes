...
> right and like so so you mentioned that like conveyor belts aren't using distance field, is there anything else apart from from the conveyors yes, oh yes i'm not running today so the limitation is also skeletal meshes, as you can see just the top part totally disappeared right and for people that may not know skeletal meshes are when you have like bone structures to to to meshes or the models in the game so like if you want to animate a character you need to tie in how like you have like a bone structure underneath anything and then when you render it you render it based on like where that bone is so when you're like running the bone will animate where the mesh like the part of the mesh is essentially and some of our buildings are animated and some parts of our building are static so one question that i've been getting
