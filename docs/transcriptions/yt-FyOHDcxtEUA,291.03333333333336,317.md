...
> and so what if what if your your path changes like if i you know it's only a thousand meters but then i build a massive wall and now it has to go a different way does that change the battery consumption it does not, we thought about doing that at first where we would just calculate the cost of batteries depending on the path the drone takes but we ended up just taking the distance between the stations and then basing it off that instead
