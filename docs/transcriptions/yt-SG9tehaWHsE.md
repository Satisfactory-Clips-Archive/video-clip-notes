...
> eventually, so the old data mine data like no that stuff is probably not up to date anymore, there was there was a few things that were data mined from the game and when in early access early even before that in alpha that just won't happen, it was like testing prototype like because, we didn't have like a proper build pipeline when we released this game so there were a bunch of stuff in that there that we were prototyping that we didn't think we're gonna make it in that made actually made it in somehow
