...
> same thing with the this goes back to some of the question about the tree regrowing another bug that we've tried to fix four times now we need to rework the whole foliage system, to be able to fix that properly and this is something that keeps creeping back every update like there's always like I think every significant update we've had there's been a couple of people running into that thing where plans regrow that's another thing where like we have to move forward and like not get stuck on that because we've tried to fix that for so long we kind of need to rework a whole system for that, for multiplayer specifically
>
> [Music]
>
> and, yeah that's just the way the cookie crumbles
