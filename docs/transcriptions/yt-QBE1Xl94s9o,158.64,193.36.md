...
> save file work in either version yes a me save file that you've made in the epic version should trademark load up in the Steam version and vice-versa we really wish we could give you more information in regards to the steam release but like I said before it's still kind of early so there's still a bunch of unknowns for us one thing that I can say however is that when we do put it out on Steam we're gonna commit equally on both versions of the game and that's also partly why we can't give it out sooner is because we need to know that we can commit to both versions if
