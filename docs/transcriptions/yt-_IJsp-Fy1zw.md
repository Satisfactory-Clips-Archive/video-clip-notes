...
> put in solar and wind power to make it ultra limited like solar only works in day and wind only works at random yeah like that's probably how we would do that, the the problem is a bit like nights are pretty short right now in the game so there wouldn't really be that much of an impact if if solar only worked during the day
>
> [Music]
>
> so but that's something that we'll have to try if we ever implement it ultra long nights like if we did add solar power like that and we would want that as a design decision then we would need to fix the rendering of the night so that's not like too dark anymore, because it is kind of difficult to see things at night
