---
title: "September 1st, 2020 Livestream Q&A: Is there going to be creative mode?"
date: "2020-09-01"
layout: transcript
topics:
    - "features/possible-features/game-modes/creative-mode"
---
# [September 1st, 2020 Livestream](../2020-09-01.md)
## Q&A: Is there going to be creative mode?
https://www.youtube.com/watch?v=zZILz6cMKbw
This question was possibly duplicated with a more recent answer: [February 8th, 2022 Livestream Q&A: Creative Mode?](./yt-si20TQaDIXs.md) [https://www.youtube.com/watch?v=si20TQaDIXs](https://www.youtube.com/watch?v=si20TQaDIXs)


### Topics
* [Features > Possible Features > Game Modes > Creative Mode](../topics/features/possible-features/game-modes/creative-mode.md)

### Transcript

> is it gonna be creative mode, yeah at some point I believe so, we don't know when because I think creative mode will happen the thing the reason why we don't have creative mode yet is because there's still stuff that we're working out or balancing with the game we're still not like 100 happy with the balancing of the game so we're still like really collecting feedback from people and we think that adding a creative mode would like kind of disrupt that a bit
>
> [Music]
>
> and for us it's very important to get that feedback from from the vanilla players so that we can update the balancing for the game once we're happy with that though we're probably going to implement like creative mode right away, because then we're probably happy with that and then people can do whatever they want and be creative and reach the bill limit much faster so yeah
