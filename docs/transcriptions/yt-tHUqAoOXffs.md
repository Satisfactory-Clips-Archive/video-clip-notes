---
title: "September 22nd, 2020 Livestream Q&A: Dedicated Servers?"
date: "2020-09-22"
layout: transcript
topics:
    - "features/multiplayer/dedicated-servers"
---
# [September 22nd, 2020 Livestream](../2020-09-22.md)
## Q&A: Dedicated Servers?
https://www.youtube.com/watch?v=tHUqAoOXffs
This question was possibly duplicated with a more recent answer: October 26th, 2021 Update 5 Launch Stream and Patch Notes Video Q&A: Dedicated Servers? [https://youtube.com/embed/Rumqu_lyapg?autoplay=1&start=1873&end=2084](https://youtube.com/embed/Rumqu_lyapg?autoplay=1&start=1873&end=2084)


### Topics
* [Features > Multiplayer > Dedicated Servers](../topics/features/multiplayer/dedicated-servers.md)

### Transcript

> some people asking about dedicated servers, no real update on that we're working on it we are working on it and no date we it's gonna be released when it's done and that will be who knows hopefully soon yeah should, we should, have a meeting about that chase maybe two maybe two meetings
>
> [Laughter]
>
>, check the status on that because I actually don't know the status on dedicated servers yeah at all pretty sure it's just, they're working on it yeah but yeah what the expectation and whatnot we'll talk to some people see
