...
> object limit any chance to display how much is left somehow, not really the the common misconception about the object limit is that people think that it's like the number of objects that are placed in the world but this is a memory thing so you know placing a foundation has a smaller object you object imprint in memory than placing a constructor has, and it's not simple to explain it because memory in programming is is very complex, it's not just like a lot of people often like describe it as like it's a it's an array of of of, ones and zeros that's memory no that's not exactly how it works, because they're there's like memory caches and and setups for how they're loaded and stuff like that but, the system that handles all that has a limit in unreal engine and, there's not much we can do about it, we can optimize for it but there's always going to be a limit, and you can even load a save file that crashes due to the object limit you can you can sometimes load that save file and sometimes you can based on what's going on in the game at that point, so kind of hard to it's it's hard to explain we're optimizing for it, and maybe in the future it will be a bit better but there's all there'll always be like a limit to it
