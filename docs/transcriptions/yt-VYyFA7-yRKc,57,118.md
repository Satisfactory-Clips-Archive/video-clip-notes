...
> nice to meet you i want to talk to you today a little bit about, how we're developing the game, specifically to the future and we go back a little bit, until the steam release basically from the time released this game in early access right we focused always on the next update who's the next thing we're going to do after steam release we thought it was time to start thinking further ahead vaguely we'll still stay flexible and how we do things but overall instead of always having our focus basically set on next step we're gonna try and have a focus set on satisfactory 1.0 the finished game what does that mean for you well hopefully not too much of a difference hopefully only positive stuff but yeah we'll still release updates kind of as usual but what kind of changes behind the scenes is that we're working on more things than usual at the same time one of those is
